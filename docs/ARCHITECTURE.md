...
- Repo-scoped Codex skills are guidance for agents, not runtime orchestration
...
`.agents/skills/` contains project-specific Codex skills. Codex discovers these skills from their `SKILL.md` files when working in this repository.

These skills help agents choose the right benchmark workflow, understand scripts, and troubleshoot runs. They are not loaded by benchmark execution code. Do not add `skill.json` runtime contracts or a custom harness that executes skills.
