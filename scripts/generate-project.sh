...
#
# Separates prompt templating from project generation for clean architecture:
# 1. Calls render-prompt.sh to template the final prompt (specs + cartridges)
# 2. Invokes harness with the final rendered prompt
#
# Supports pluggable harnesses (opencode, pi) with provider-specific model
# routing and harness-agnostic session management.
#
# Usage:
...
#     --backend      Backend framework (spring-boot, node-js, quarkus)
#     --frontend     Frontend framework (angular, react)
#
#   Optional:
#     --output-dir   Override output directory (auto-generated by default)
#     --harness      Generation harness (default: opencode; valid: opencode, pi)
...
#     --template     Custom prompt template path
...
#
# Architecture:
#   - Output directory auto-generated: WORKSPACE/{HARNESS}-{MODEL_SLUG}/{LEVEL}
#   - Prompt rendering (specs + cartridges) handled by render-prompt.sh
#   - Harness invocation uses only the final rendered prompt
#   - Session tracking and retry logic managed separately
################################################################################
...
if ! "$SCRIPT_DIR/render-prompt.sh" \
  ...
  --frontend-cartridge "$FRONTEND_CARTRIDGE" \
  --level "$LEVEL" \
  --backend "$BACKEND" \
  --frontend "$FRONTEND" \
...
$GEN_PROMPT"
    ...
    kilo-code)
      ...
      echo "  See implementation guide: .agents/skills/harness-${HARNESS}/SKILL.md" >&2
      exit 1
      ;;
    *)
