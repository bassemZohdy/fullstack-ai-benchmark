...
OpenCode creates a resumable session for every run. `generate-project.sh` captures and persists the session ID automatically after each attempt using `.agents/skills/harness-opencode/helpers/capture-session.sh`.
...
- Session capture helper: `.agents/skills/harness-opencode/helpers/capture-session.sh`
- Session/timeout/retry deep-dive: `.agents/skills/project-generation/EXTENDED.md`
